...
export default function DailyTrack() {
  ...
  return (
    <div className="container py-24">
      ...
      <div className="mb-4">
        <h2 className="text-2xl font-bold"  style={{ fontFamily: "poppins" }}>Daily Tracker</h2>
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className={`h-6 w-6 flex items-center justify-center rounded-full border border-[#d1d1cf] ${
                  activity.completed ? "bg-[#FE9A00] hover:bg-[#FE9A00]" : "hover:bg-gray-100"
                }`}
              >
                {activity.completed && <Check className="h-3 w-3 text-white" />}
              </button>
              <span className={`${activity.completed ? "line-through text-gray-400" : "text-gray-900"}`}>
                {activity.name}
              </span>
            </div>
            <button
