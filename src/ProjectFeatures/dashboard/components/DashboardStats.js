function DashboardStats({title, icon, value, description, colorIndex}){
    ...
    return(
        <div className="stats shadow">
            <div className="stat">
                <div className={`stat-figure dark:text-primary text-${COLORS[colorIndex%2]}`}>{icon}</div>
                <div className="stat-title dark:text-primary">{title}</div>
                <div className={`stat-value dark:text-primary text-${COLORS[colorIndex%2]}`}>{value}</div>
